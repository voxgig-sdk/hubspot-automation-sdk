...
// HubspotAutomation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = HubspotAutomationOmni::makeStructRunner(
    ...
    {
        return HubspotAutomationOmni::objmodel(HubspotAutomationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return HubspotAutomationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return HubspotAutomationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(HubspotAutomationOmni::structfix($now, Struct::undef()))
                        !== json_encode(HubspotAutomationOmni::structfix($val, Struct::undef()))) {
