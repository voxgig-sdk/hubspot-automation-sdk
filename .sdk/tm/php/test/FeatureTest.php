...
// HubspotAutomation SDK feature test
//
...
class FtCtrl extends HubspotAutomationControl
{
...
class FtClient
{
    ...
    private HubspotAutomationUtility $utility;

    public function __construct(HubspotAutomationUtility $utility, array $options)
    {
    ...
    public function get_utility(): HubspotAutomationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HubspotAutomationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HubspotAutomationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HubspotAutomationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HubspotAutomationConfig::make_feature($name);
    ...
    private static function buildUrl(HubspotAutomationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HubspotAutomationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HubspotAutomationResponse($fetched);
        }
    ...
    private function fail(HubspotAutomationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HubspotAutomationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
