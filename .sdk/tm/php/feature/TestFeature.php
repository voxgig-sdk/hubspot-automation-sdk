...
// HubspotAutomation SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HubspotAutomationTestFeature extends HubspotAutomationBaseFeature
{
    ...
    public function init(HubspotAutomationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HubspotAutomationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HubspotAutomationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HubspotAutomationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HubspotAutomationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HubspotAutomationParam::call($ctx, $k);
