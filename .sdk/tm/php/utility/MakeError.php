...
class HubspotAutomationMakeError
{
    public static function call(?HubspotAutomationContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HubspotAutomationContext([], null);
        }
        $op = $ctx->op ?? new HubspotAutomationOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HubspotAutomationResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HubspotAutomationError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HubspotAutomationError('', $msg, $ctx);
        ...
        if ($err instanceof HubspotAutomationError) {
