...
from projectname_sdk.core.operation import HubspotAutomationOperation
...
_runner = makeRunner(TEST_JSON_FILE, HubspotAutomationSDK.test(None, None))
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return HubspotAutomationError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_exists(self):
        client = HubspotAutomationSDK.test(None, None)
        utility = client._utility
    ...
    def test_feature_hook_basic(self):
        hook_client = HubspotAutomationSDK.test(None, None)
        hook_utility = hook_client._utility
        ctx = _make_test_ctx(hook_client, hook_utility)

        state = {"called": False}

        class TestHookFeature(HubspotAutomationBaseFeature):
    ...
    def test_feature_init_basic(self):
        init_client = HubspotAutomationSDK.test(None, None)
        ...
        class TestInitFeature(HubspotAutomationBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(HubspotAutomationBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = HubspotAutomationSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = HubspotAutomationSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = HubspotAutomationSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = HubspotAutomationSDK.test(None, setup.get('a') or {})
    ...
    def test_operator_basic(self):
        def subject(vin):
            if not isinstance(vin, dict):
                vin = {}
            op = HubspotAutomationOperation(vin)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = HubspotAutomationSDK.test(None, setup.get('a') or {})
