...
from projectname_sdk import HubspotAutomationSDK
from projectname_sdk.core.error import HubspotAutomationError
from projectname_sdk.core.result import HubspotAutomationResult
from projectname_sdk.core.response import HubspotAutomationResponse
from projectname_sdk.core.spec import HubspotAutomationSpec
from projectname_sdk.feature.base_feature import HubspotAutomationBaseFeature


def _client():
    return HubspotAutomationSDK.test(None, None)
...
def _resp(status, data=None, headers=None):
    ...
    return HubspotAutomationResponse({
        "status": status,
...
class TestMakePointAndMakeSpec:
    ...
    def test_make_spec_short_circuits_a_feature_supplied_spec(self):
        client = _client()
        ctx = _ctx(client, opname="load")
        preset = HubspotAutomationSpec({"method": "GET"})
        ctx.out["spec"] = preset
...
class TestMakeResponse:
    ...
    def test_guards_missing_spec_response_result(self):
        ...
        ctx.result = HubspotAutomationResult({})
...
class TestMakeResult:
    ...
    def test_list_op_wraps_resdata_into_entity_instances(self):
        ...
        ctx.entity = entity
        ctx.spec = _full_spec()
        ctx.result = HubspotAutomationResult({"resdata": [{"a": 1}, {"a": 2}]})
    ...
    def test_an_empty_list_yields_an_empty_resdata_array(self):
        ...
        ctx.spec = _full_spec()
        ctx.result = HubspotAutomationResult({"resdata": []})
        result, err = client._utility.make_result(ctx)
        assert err is None
    ...
    def test_short_circuits_on_a_preset_result(self):
        client = _client()
        ctx = _ctx(client)
        preset = HubspotAutomationResult({"ok": True})
...
class TestMakeRequest:
    ...
    def test_short_circuits_a_feature_supplied_request(self):
        client = _client()
        ctx = _ctx(client)
        preset = HubspotAutomationResponse({"status": 201, "statusText": "OK"})
...
class TestMakeErrorAndDone:
    ...
    def test_make_error_returns_resdata_when_throw_is_disabled(self):
        ...
        ctx.result = HubspotAutomationResult({"ok": False, "resdata": "fallback"})
...
class TestFeatureOrdering:
    ...
    def _named_feature(self, name):
        feature = HubspotAutomationBaseFeature()
        feature.name = name
        return feature
...
class TestPrepareAuth:
    ...
    def _auth_ctx(self, client, options, headers):
        ...
        ctx.spec = None if headers is None else HubspotAutomationSpec(
            {"headers": headers})
        return ctx
...
class TestResultHelpers:
    ...
    def test_result_headers_with_no_headers_yields_an_empty_map(self):
        client = _client()
        ctx = _ctx(client)
        ctx.response = HubspotAutomationResponse({"status": 200})
        ctx.result = HubspotAutomationResult({})
