// Command HubspotAutomation-cli is a CLI + REPL for the HubspotAutomation SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HUBSPOT_AUTOMATION_APIKEY (api key), HUBSPOT_AUTOMATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HUBSPOT_AUTOMATION_APIKEY carries the API key and
	// HUBSPOT_AUTOMATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HUBSPOT_AUTOMATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HUBSPOT_AUTOMATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHubspotAutomationSDK(opts)
