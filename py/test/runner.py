# HubspotAutomation SDK test runner
...
class HubspotAutomationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HubspotAutomationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HubspotAutomationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HubspotAutomationTestRunner.getenv("HUBSPOT_AUTOMATION_TEST_LIVE")
        override = HubspotAutomationTestRunner.getenv("HUBSPOT_AUTOMATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HubspotAutomationTestRunner.getenv(key)
        ...
        explain = HubspotAutomationTestRunner.getenv("HUBSPOT_AUTOMATION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HUBSPOT_AUTOMATION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HubspotAutomationTestRunner._test_control is not None:
            return HubspotAutomationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HubspotAutomationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HubspotAutomationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HubspotAutomationTestRunner.load_test_control()
...
def load_env_local():
    HubspotAutomationTestRunner.load_env_local()


def env_override(m):
    return HubspotAutomationTestRunner.env_override(m)


def entity_data(v):
    return HubspotAutomationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HubspotAutomationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HubspotAutomationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HubspotAutomationTestRunner.load_test_control()


def live_client_options():
    return HubspotAutomationTestRunner.live_client_options()


def live_delay_ms():
    return HubspotAutomationTestRunner.live_delay_ms()
