...
class TestEmailTemplatesPublicTemplateDirect:

    def test_should_direct_load_email_templates_public_template(self):
        setup = _email_templates_public_template_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-email_templates_public_template", "live" if setup["live"] else "unit")
        if _skip:
...
def _email_templates_public_template_direct_setup(mockres):
