    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HUBSPOT_AUTOMATION_TEST_FUNCTION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        function_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.function")))
        function_ref01_data = None
        if len(function_ref01_data_raw) > 0:
...
def _function_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "HUBSPOT_AUTOMATION_TEST_FUNCTION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HUBSPOT_AUTOMATION_TEST_FUNCTION_ENTID"))
