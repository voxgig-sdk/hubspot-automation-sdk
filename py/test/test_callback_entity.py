...
class TestCallbackEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Callback(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _callback_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "callback." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        callback_ref01_ent = client.Callback(None)
        callback_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.callback"), "callback_ref01"))

        callback_ref01_data = helpers.to_map(runner.entity_data(callback_ref01_ent.create(callback_ref01_data, None)))
        assert callback_ref01_data is not None



def _callback_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/callback/CallbackTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID"))
