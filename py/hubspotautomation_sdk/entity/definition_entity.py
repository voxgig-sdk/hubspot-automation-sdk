...
class DefinitionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DefinitionEntity(self._client, opts)
    ...
    def create(self, reqdata: DefinitionCreateData, ctrl=None) -> Definition:
        utility = self._utility
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Definition:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Definition().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
