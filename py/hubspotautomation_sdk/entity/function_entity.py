...
class FunctionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return FunctionEntity(self._client, opts)
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Function:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Function().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
