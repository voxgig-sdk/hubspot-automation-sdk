...
class AutomationV4ApiFlowEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AutomationV4ApiFlowEntity(self._client, opts)
    ...
    def data_get(self) -> AutomationV4ApiFlow:
    ...
    def match_get(self) -> AutomationV4ApiFlow:
    ...
    def load(self, reqmatch=None, ctrl=None) -> AutomationV4ApiFlow:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AutomationV4ApiFlow().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: AutomationV4ApiFlowCreateData, ctrl=None) -> AutomationV4ApiFlow:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
    ...
    def update(self, reqdata: AutomationV4ApiFlowUpdateData, ctrl=None) -> AutomationV4ApiFlow:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
