# hubspot-automation-mcp

[MCP](https://modelcontextprotocol.io) server exposing the HubspotAutomation SDK as
two agent tools — `hubspot-automation_list` and `hubspot-automation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hubspot-automation-mcp)
make build
...
claude mcp add --scope user hubspot-automation \
  -- /absolute/path/to/hubspot-automation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hubspot-automation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hubspot-automation_list: first page of records
{ "entity": "actions_v4_collection_response_public_action_function_identifier_no" }
{ "entity": "actions_v4_collection_response_public_action_function_identifier_no", "query": { } }

// hubspot-automation_load: one record by id
{ "entity": "actions_v4_collection_response_public_action_definition_forward", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hubspot-automation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hubspot-automation \
     -- "$PWD"/dist/*/hubspot-automation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hubspot-automation_list` and `hubspot-automation_load` tools now appear
   in new sessions. Ask the agent to *"list actions_v4_collection_response_public_action_function_identifier_no using hubspot-automation"*
   and it calls `hubspot-automation_list` with `{"entity":"actions_v4_collection_response_public_action_function_identifier_no"}`.
...
### Call the `hubspot-automation_list` tool
...
### Call the `hubspot-automation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hubspot-automation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hubspot-automation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./hubspot-automation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hubspot-automation_load","arguments":{"entity":"actions_v4_collection_response_public_action_definition_forward","query":{"id":1}}}}'
```
