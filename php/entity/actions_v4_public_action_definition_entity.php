...
// HubspotAutomation SDK ActionsV4PublicActionDefinition entity
...
class ActionsV4PublicActionDefinitionEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "actions_v4_public_action_definition";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new ActionsV4PublicActionDefinitionEntity($this->_client, $opts);
    }

    /**
     * @param ActionsV4PublicActionDefinition|array $args ActionsV4PublicActionDefinition data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = HubspotAutomationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return ActionsV4PublicActionDefinition|array The current ActionsV4PublicActionDefinition data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of ActionsV4PublicActionDefinition fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = HubspotAutomationHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of ActionsV4PublicActionDefinition fields).
     */
    ...
    /**
     * Load a single ActionsV4PublicActionDefinition.
     *
     * @param ActionsV4PublicActionDefinitionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed ActionsV4PublicActionDefinitionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ActionsV4PublicActionDefinition|array The loaded ActionsV4PublicActionDefinition as an assoc-array at the
     *   SDK boundary; throws HubspotAutomationError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = HubspotAutomationHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * Create a new ActionsV4PublicActionDefinition.
     *
     * @param ActionsV4PublicActionDefinitionCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ActionsV4PublicActionDefinitionCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ActionsV4PublicActionDefinition|array The created ActionsV4PublicActionDefinition as an assoc-array at the
     *   SDK boundary; throws HubspotAutomationError on failure (item-5 convention).
     */
    ...
    /**
     * Update an existing ActionsV4PublicActionDefinition.
     *
     * @param ActionsV4PublicActionDefinitionUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ActionsV4PublicActionDefinitionUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return ActionsV4PublicActionDefinition|array The updated ActionsV4PublicActionDefinition as an assoc-array at the
     *   SDK boundary; throws HubspotAutomationError on failure (item-5 convention).
     */
