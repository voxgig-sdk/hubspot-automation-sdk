...
class HubspotAutomationMakeRequest
{
    public static function call(HubspotAutomationContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new HubspotAutomationResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new HubspotAutomationResponse($fetched);
        } else {
