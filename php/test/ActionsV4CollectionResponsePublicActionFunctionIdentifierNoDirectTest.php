...
// ActionsV4CollectionResponsePublicActionFunctionIdentifierNo direct test
...
class ActionsV4CollectionResponsePublicActionFunctionIdentifierNoDirectTest extends TestCase
{
    public function test_direct_list_actions_v4_collection_response_public_action_function_identifier_no(): void
    {
        $setup = actions_v4_collection_response_public_action_function_identifier_no_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-actions_v4_collection_response_public_action_function_identifier_no", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["app01", "definition01"] as $_liveKey) {
...
function actions_v4_collection_response_public_action_function_identifier_no_direct_setup($mockres)
{
