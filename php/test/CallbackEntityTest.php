...
class CallbackEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function callback_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["callback01", "callback02", "callback03", "2026_0901", "2026_0902", "2026_0903"] as $k) {
    ...
    $entid_env_raw = getenv("HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID");
