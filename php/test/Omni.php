...
//    only - a typed HubspotAutomationContext there would make every ctx
...
final class HubspotAutomationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HubspotAutomationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HubspotAutomationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HubspotAutomationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HubspotAutomationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HubspotAutomationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HubspotAutomationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HubspotAutomationError(is_string($code) ? $code : '', $msg);
    }
