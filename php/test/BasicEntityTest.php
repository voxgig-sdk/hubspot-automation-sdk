...
class BasicEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Basic(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = basic_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach ([] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "basic." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HUBSPOT_AUTOMATION_TEST_BASIC_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $basic_ref01_data = null;
        if (count($basic_ref01_data_raw) > 0) {
...
function basic_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/basic/BasicTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["basic01", "basic02", "basic03", "sequence01", "sequence02", "sequence03", "flow01", "flow02", "flow03"] as $k) {
    ...
    $entid_env_raw = getenv("HUBSPOT_AUTOMATION_TEST_BASIC_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["HUBSPOT_AUTOMATION_TEST_BASIC_ENTID"]);
