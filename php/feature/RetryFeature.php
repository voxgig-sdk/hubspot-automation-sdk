...
// HubspotAutomation SDK retry feature
...
class HubspotAutomationRetryFeature extends HubspotAutomationBaseFeature
{
    ...
    public function init(HubspotAutomationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HubspotAutomationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HubspotAutomationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HubspotAutomationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
