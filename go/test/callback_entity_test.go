...
func TestCallbackEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func callbackBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"callback01", "callback02", "callback03", "2026_0901", "2026_0902", "2026_0903"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HUBSPOT_AUTOMATION_TEST_CALLBACK_ENTID"])
