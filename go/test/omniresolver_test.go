...
import (
	...
	omni "github.com/voxgig-sdk/hubspot-automation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HubspotAutomationError); is {
				out := map[string]any{
					"name":    "HubspotAutomationError",
...
func omniCtx(arg any, client *sdk.HubspotAutomationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HubspotAutomationSDK); is {
			client = live
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.HubspotAutomationError{Msg: msg}
			}
		}
	}
...
func omniErrArg(arg any) error {
	...
	return &sdk.HubspotAutomationError{Msg: msg, Code: code}
}
