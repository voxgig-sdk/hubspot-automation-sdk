...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("HUBSPOT_AUTOMATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("HUBSPOT_AUTOMATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("HUBSPOT_AUTOMATION_TEST_EXPLAIN"); explain != "" {
