...
func TestSequenceDirect(t *testing.T) {
	t.Run("direct-list-sequence", func(t *testing.T) {
		setup := sequenceDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-sequence", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "automation/sequences/2026-09/serviceaccounts/sequences",
			"method": "GET",
	...
	t.Run("direct-load-sequence", func(t *testing.T) {
		setup := sequenceDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-sequence", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "automation/sequences/2026-09/serviceaccounts/sequences/{id}",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type sequenceDirectSetupResult struct {
...
func sequenceDirectSetup(mockres any) *sequenceDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"HUBSPOT_AUTOMATION_TEST_SEQUENCE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HUBSPOT_AUTOMATION_TEST_SEQUENCE_ENTID"]; ok {
		...
		return &sequenceDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &sequenceDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
