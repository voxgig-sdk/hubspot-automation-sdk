...
describe("ActionsV4CollectionResponsePublicActionFunctionIdentifierNoDirect", function()
  it("should direct-list-actions_v4_collection_response_public_action_function_identifier_no", function()
    local setup = actions_v4_collection_response_public_action_function_identifier_no_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-actions_v4_collection_response_public_action_function_identifier_no", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"app01", "definition01"}) do
