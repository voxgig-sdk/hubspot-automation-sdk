...
describe("SequenceDirect", function()
  it("should direct-list-sequence", function()
    local setup = sequence_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-sequence", setup.live and "live" or "unit")
  ...
  it("should direct-load-sequence", function()
    local setup = sequence_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-sequence", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
