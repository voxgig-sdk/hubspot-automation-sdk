...
describe("SequencesPublicSequencePerformanceDirect", function()
  it("should direct-list-sequences_public_sequence_performance", function()
    local setup = sequences_public_sequence_performance_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-sequences_public_sequence_performance", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"sequence01"}) do
