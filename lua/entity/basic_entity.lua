...
local BasicEntity = {}
BasicEntity.__index = BasicEntity


function BasicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BasicEntity)
  self._name = "basic"
  self._client = client
...
function BasicEntity:get_name()
  return self._name
end


function BasicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BasicEntity.new(self._client, opts)
end
...
function BasicEntity:mark_deleted()
  self._deleted = true
end


function BasicEntity:deleted()
  return true == self._deleted
end


function BasicEntity:data_set(args)
  if args ~= nil then
...
function BasicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BasicEntity:match_set(args)
  if args ~= nil then
...
function BasicEntity:match_get()
...
function BasicEntity:stream(action, args, callopts)
...
---@param reqmatch BasicRemoveMatch
---@param ctrl? table
---@return Basic
---@return string? err
function BasicEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function BasicEntity:_run_op(ctx, post_done)
  local utility = self._utility
