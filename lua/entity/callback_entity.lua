...
CallbackEntity.__index = CallbackEntity


function CallbackEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CallbackEntity)
  self._name = "callback"
  self._client = client
...
function CallbackEntity:get_name()
  return self._name
end


function CallbackEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CallbackEntity.new(self._client, opts)
end
...
function CallbackEntity:mark_deleted()
  self._deleted = true
end


function CallbackEntity:deleted()
  return true == self._deleted
end


function CallbackEntity:data_set(args)
  if args ~= nil then
...
function CallbackEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CallbackEntity:match_set(args)
  if args ~= nil then
...
function CallbackEntity:match_get()
...
function CallbackEntity:stream(action, args, callopts)
...
---@param reqdata CallbackCreateData
---@param ctrl? table
---@return Callback
---@return string? err
function CallbackEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function CallbackEntity:_run_op(ctx, post_done)
  local utility = self._utility
