-- HubspotAutomation SDK ActionsV4PublicActionDefinitionRequiresObject entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActionsV4PublicActionDefinitionRequiresObjectEntity = {}
ActionsV4PublicActionDefinitionRequiresObjectEntity.__index = ActionsV4PublicActionDefinitionRequiresObjectEntity


function ActionsV4PublicActionDefinitionRequiresObjectEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsV4PublicActionDefinitionRequiresObjectEntity)
  self._name = "actions_v4_public_action_definition_requires_object"
  self._client = client
...
function ActionsV4PublicActionDefinitionRequiresObjectEntity:get_name()
  return self._name
end


function ActionsV4PublicActionDefinitionRequiresObjectEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsV4PublicActionDefinitionRequiresObjectEntity.new(self._client, opts)
end
...
function ActionsV4PublicActionDefinitionRequiresObjectEntity:mark_deleted()
  self._deleted = true
end


function ActionsV4PublicActionDefinitionRequiresObjectEntity:deleted()
  return true == self._deleted
end


function ActionsV4PublicActionDefinitionRequiresObjectEntity:data_set(args)
  if args ~= nil then
...
function ActionsV4PublicActionDefinitionRequiresObjectEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsV4PublicActionDefinitionRequiresObjectEntity:match_set(args)
  if args ~= nil then
...
function ActionsV4PublicActionDefinitionRequiresObjectEntity:match_get()
...
function ActionsV4PublicActionDefinitionRequiresObjectEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsV4PublicActionDefinitionRequiresObjectLoadMatch
---@param ctrl? table
---@return ActionsV4PublicActionDefinitionRequiresObject
---@return string? err
function ActionsV4PublicActionDefinitionRequiresObjectEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsV4PublicActionDefinitionRequiresObjectEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsV4PublicActionDefinitionRequiresObjectEntity
