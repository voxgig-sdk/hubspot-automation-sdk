-- HubspotAutomation SDK AutomationV4BatchResponseApiFlow entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AutomationV4BatchResponseApiFlowEntity = {}
AutomationV4BatchResponseApiFlowEntity.__index = AutomationV4BatchResponseApiFlowEntity


function AutomationV4BatchResponseApiFlowEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AutomationV4BatchResponseApiFlowEntity)
  self._name = "automation_v4_batch_response_api_flow"
  self._client = client
...
function AutomationV4BatchResponseApiFlowEntity:get_name()
  return self._name
end


function AutomationV4BatchResponseApiFlowEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AutomationV4BatchResponseApiFlowEntity.new(self._client, opts)
end
...
function AutomationV4BatchResponseApiFlowEntity:mark_deleted()
  self._deleted = true
end


function AutomationV4BatchResponseApiFlowEntity:deleted()
  return true == self._deleted
end


function AutomationV4BatchResponseApiFlowEntity:data_set(args)
  if args ~= nil then
...
function AutomationV4BatchResponseApiFlowEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AutomationV4BatchResponseApiFlowEntity:match_set(args)
  if args ~= nil then
...
function AutomationV4BatchResponseApiFlowEntity:match_get()
...
function AutomationV4BatchResponseApiFlowEntity:stream(action, args, callopts)
...
---@param reqdata AutomationV4BatchResponseApiFlowCreateData
---@param ctrl? table
---@return AutomationV4BatchResponseApiFlow
---@return string? err
function AutomationV4BatchResponseApiFlowEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function AutomationV4BatchResponseApiFlowEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AutomationV4BatchResponseApiFlowEntity
