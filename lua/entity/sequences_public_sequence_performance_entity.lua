-- HubspotAutomation SDK SequencesPublicSequencePerformance entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SequencesPublicSequencePerformanceEntity = {}
SequencesPublicSequencePerformanceEntity.__index = SequencesPublicSequencePerformanceEntity


function SequencesPublicSequencePerformanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SequencesPublicSequencePerformanceEntity)
  self._name = "sequences_public_sequence_performance"
  self._client = client
...
function SequencesPublicSequencePerformanceEntity:get_name()
  return self._name
end


function SequencesPublicSequencePerformanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SequencesPublicSequencePerformanceEntity.new(self._client, opts)
end
...
function SequencesPublicSequencePerformanceEntity:mark_deleted()
  self._deleted = true
end


function SequencesPublicSequencePerformanceEntity:deleted()
  return true == self._deleted
end


function SequencesPublicSequencePerformanceEntity:data_set(args)
  if args ~= nil then
...
function SequencesPublicSequencePerformanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SequencesPublicSequencePerformanceEntity:match_set(args)
  if args ~= nil then
...
function SequencesPublicSequencePerformanceEntity:match_get()
...
function SequencesPublicSequencePerformanceEntity:stream(action, args, callopts)
...
---@param reqmatch SequencesPublicSequencePerformanceListMatch
---@param ctrl? table
---@return SequencesPublicSequencePerformance[]
---@return string? err
function SequencesPublicSequencePerformanceEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function SequencesPublicSequencePerformanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return SequencesPublicSequencePerformanceEntity
