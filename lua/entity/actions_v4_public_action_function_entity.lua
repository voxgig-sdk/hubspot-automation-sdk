...
local ActionsV4PublicActionFunctionEntity = {}
ActionsV4PublicActionFunctionEntity.__index = ActionsV4PublicActionFunctionEntity


function ActionsV4PublicActionFunctionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsV4PublicActionFunctionEntity)
  self._name = "actions_v4_public_action_function"
  self._client = client
...
function ActionsV4PublicActionFunctionEntity:get_name()
  return self._name
end


function ActionsV4PublicActionFunctionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsV4PublicActionFunctionEntity.new(self._client, opts)
end
...
function ActionsV4PublicActionFunctionEntity:mark_deleted()
  self._deleted = true
end


function ActionsV4PublicActionFunctionEntity:deleted()
  return true == self._deleted
end


function ActionsV4PublicActionFunctionEntity:data_set(args)
  if args ~= nil then
...
function ActionsV4PublicActionFunctionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsV4PublicActionFunctionEntity:match_set(args)
  if args ~= nil then
...
function ActionsV4PublicActionFunctionEntity:match_get()
...
function ActionsV4PublicActionFunctionEntity:stream(action, args, callopts)
...
---@param reqmatch ActionsV4PublicActionFunctionLoadMatch
---@param ctrl? table
---@return ActionsV4PublicActionFunction
---@return string? err
function ActionsV4PublicActionFunctionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ActionsV4PublicActionFunctionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsV4PublicActionFunctionEntity
