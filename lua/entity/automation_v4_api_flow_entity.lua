...
local AutomationV4ApiFlowEntity = {}
AutomationV4ApiFlowEntity.__index = AutomationV4ApiFlowEntity


function AutomationV4ApiFlowEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AutomationV4ApiFlowEntity)
  self._name = "automation_v4_api_flow"
  self._client = client
...
function AutomationV4ApiFlowEntity:get_name()
  return self._name
end


function AutomationV4ApiFlowEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AutomationV4ApiFlowEntity.new(self._client, opts)
end
...
function AutomationV4ApiFlowEntity:mark_deleted()
  self._deleted = true
end


function AutomationV4ApiFlowEntity:deleted()
  return true == self._deleted
end


function AutomationV4ApiFlowEntity:data_set(args)
  if args ~= nil then
...
function AutomationV4ApiFlowEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AutomationV4ApiFlowEntity:match_set(args)
  if args ~= nil then
...
function AutomationV4ApiFlowEntity:match_get()
...
function AutomationV4ApiFlowEntity:stream(action, args, callopts)
...
---@param reqmatch AutomationV4ApiFlowLoadMatch
---@param ctrl? table
---@return AutomationV4ApiFlow
---@return string? err
function AutomationV4ApiFlowEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata AutomationV4ApiFlowCreateData
---@param ctrl? table
---@return AutomationV4ApiFlow
---@return string? err
function AutomationV4ApiFlowEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata AutomationV4ApiFlowUpdateData
---@param ctrl? table
---@return AutomationV4ApiFlow
---@return string? err
function AutomationV4ApiFlowEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AutomationV4ApiFlowEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AutomationV4ApiFlowEntity
