-- HubspotAutomation SDK SequencesPublicSequenceEnrollment entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SequencesPublicSequenceEnrollmentEntity = {}
SequencesPublicSequenceEnrollmentEntity.__index = SequencesPublicSequenceEnrollmentEntity


function SequencesPublicSequenceEnrollmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SequencesPublicSequenceEnrollmentEntity)
  self._name = "sequences_public_sequence_enrollment"
  self._client = client
...
function SequencesPublicSequenceEnrollmentEntity:get_name()
  return self._name
end


function SequencesPublicSequenceEnrollmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SequencesPublicSequenceEnrollmentEntity.new(self._client, opts)
end
...
function SequencesPublicSequenceEnrollmentEntity:mark_deleted()
  self._deleted = true
end


function SequencesPublicSequenceEnrollmentEntity:deleted()
  return true == self._deleted
end


function SequencesPublicSequenceEnrollmentEntity:data_set(args)
  if args ~= nil then
...
function SequencesPublicSequenceEnrollmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SequencesPublicSequenceEnrollmentEntity:match_set(args)
  if args ~= nil then
...
function SequencesPublicSequenceEnrollmentEntity:match_get()
...
function SequencesPublicSequenceEnrollmentEntity:stream(action, args, callopts)
...
---@param reqmatch SequencesPublicSequenceEnrollmentLoadMatch
---@param ctrl? table
---@return SequencesPublicSequenceEnrollment
---@return string? err
function SequencesPublicSequenceEnrollmentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SequencesPublicSequenceEnrollmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return SequencesPublicSequenceEnrollmentEntity
