-- HubspotAutomation SDK ActionsV4PublicActionFunctionIdentifier entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ActionsV4PublicActionFunctionIdentifierEntity = {}
ActionsV4PublicActionFunctionIdentifierEntity.__index = ActionsV4PublicActionFunctionIdentifierEntity


function ActionsV4PublicActionFunctionIdentifierEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActionsV4PublicActionFunctionIdentifierEntity)
  self._name = "actions_v4_public_action_function_identifier"
  self._client = client
...
function ActionsV4PublicActionFunctionIdentifierEntity:get_name()
  return self._name
end


function ActionsV4PublicActionFunctionIdentifierEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActionsV4PublicActionFunctionIdentifierEntity.new(self._client, opts)
end
...
function ActionsV4PublicActionFunctionIdentifierEntity:mark_deleted()
  self._deleted = true
end


function ActionsV4PublicActionFunctionIdentifierEntity:deleted()
  return true == self._deleted
end


function ActionsV4PublicActionFunctionIdentifierEntity:data_set(args)
  if args ~= nil then
...
function ActionsV4PublicActionFunctionIdentifierEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActionsV4PublicActionFunctionIdentifierEntity:match_set(args)
  if args ~= nil then
...
function ActionsV4PublicActionFunctionIdentifierEntity:match_get()
...
function ActionsV4PublicActionFunctionIdentifierEntity:stream(action, args, callopts)
...
---@param reqdata ActionsV4PublicActionFunctionIdentifierUpdateData
---@param ctrl? table
---@return ActionsV4PublicActionFunctionIdentifier
---@return string? err
function ActionsV4PublicActionFunctionIdentifierEntity:update(reqdata, ctrl)
  local utility = self._utility
...
function ActionsV4PublicActionFunctionIdentifierEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return ActionsV4PublicActionFunctionIdentifierEntity
