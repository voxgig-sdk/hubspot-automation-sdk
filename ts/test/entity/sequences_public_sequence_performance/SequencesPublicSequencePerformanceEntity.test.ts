...
describe('SequencesPublicSequencePerformanceEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'sequences_public_sequence_performance.' + op, live)) return
    }
    ...
    let sequences_public_sequence_performance_ref01_data = Object.values(setup.data.existing.sequences_public_sequence_performance)[0] as any

    // LIST
    const sequences_public_sequence_performance_ref01_ent = client.SequencesPublicSequencePerformance()
    const sequences_public_sequence_performance_ref01_match: any = {}
    sequences_public_sequence_performance_ref01_match['sequence_id'] = setup.idmap['sequence01']

    const sequences_public_sequence_performance_ref01_list = (await sequences_public_sequence_performance_ref01_ent.list(sequences_public_sequence_performance_ref01_match)).map((e: any) => e.data())
