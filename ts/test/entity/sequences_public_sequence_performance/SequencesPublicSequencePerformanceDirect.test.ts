...
describe('SequencesPublicSequencePerformanceDirect', async () => {
  ...
  test('direct-list-sequences_public_sequence_performance', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-sequences_public_sequence_performance', setup.live)) return
    if (skipIfMissingIds(t, setup, ["sequence01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_SEQUENCES_PUBLIC_SEQUENCE_PERFORMANCE_ENTID']
