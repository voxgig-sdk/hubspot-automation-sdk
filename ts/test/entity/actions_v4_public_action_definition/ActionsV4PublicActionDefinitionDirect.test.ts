...
describe('ActionsV4PublicActionDefinitionDirect', async () => {
  ...
  test('direct-load-actions_v4_public_action_definition', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-actions_v4_public_action_definition', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_ACTIONS_V4_PUBLIC_ACTION_DEFINITION_ENTID']
