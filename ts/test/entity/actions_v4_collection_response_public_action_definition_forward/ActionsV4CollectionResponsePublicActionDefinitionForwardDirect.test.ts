...
describe('ActionsV4CollectionResponsePublicActionDefinitionForwardDirect', async () => {
  ...
  test('direct-load-actions_v4_collection_response_public_action_definition_forward', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-actions_v4_collection_response_public_action_definition_forward', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_ACTIONS_V4_COLLECTION_RESPONSE_PUBLIC_ACTION_DEFINITION_FORWARD_ENTID']
