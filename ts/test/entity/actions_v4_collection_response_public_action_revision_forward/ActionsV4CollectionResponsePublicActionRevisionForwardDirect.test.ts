...
describe('ActionsV4CollectionResponsePublicActionRevisionForwardDirect', async () => {
  ...
  test('direct-list-actions_v4_collection_response_public_action_revision_forward', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-actions_v4_collection_response_public_action_revision_forward', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_ACTIONS_V4_COLLECTION_RESPONSE_PUBLIC_ACTION_REVISION_FORWARD_ENTID']
