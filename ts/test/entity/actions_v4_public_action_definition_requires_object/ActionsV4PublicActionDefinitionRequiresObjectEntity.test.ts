...
describe('ActionsV4PublicActionDefinitionRequiresObjectEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'actions_v4_public_action_definition_requires_object.' + op, live)) return
    }
    ...
    let actions_v4_public_action_definition_requires_object_ref01_data = Object.values(setup.data.existing.actions_v4_public_action_definition_requires_object)[0] as any
