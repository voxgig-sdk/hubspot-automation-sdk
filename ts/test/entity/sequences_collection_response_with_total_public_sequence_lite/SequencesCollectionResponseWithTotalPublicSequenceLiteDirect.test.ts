...
describe('SequencesCollectionResponseWithTotalPublicSequenceLiteDirect', async () => {
  ...
  test('direct-list-sequences_collection_response_with_total_public_sequence_lite', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-sequences_collection_response_with_total_public_sequence_lite', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_SEQUENCES_COLLECTION_RESPONSE_WITH_TOTAL_PUBLIC_SEQUENCE_LITE_ENTID']
