...
describe('EmailTemplatesCollectionResponsePublicFolderForwardPagingDirect', async () => {
  ...
  test('direct-list-email_templates_collection_response_public_folder_forward_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-email_templates_collection_response_public_folder_forward_paging', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_EMAIL_TEMPLATES_COLLECTION_RESPONSE_PUBLIC_FOLDER_FORWARD_PAGING_ENTID']
