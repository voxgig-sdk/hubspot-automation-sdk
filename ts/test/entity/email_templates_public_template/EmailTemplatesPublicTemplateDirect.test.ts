...
describe('EmailTemplatesPublicTemplateDirect', async () => {
  ...
  test('direct-load-email_templates_public_template', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-email_templates_public_template', setup.live)) return
    if (skipIfMissingIds(t, setup, ["template_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_EMAIL_TEMPLATES_PUBLIC_TEMPLATE_ENTID']
