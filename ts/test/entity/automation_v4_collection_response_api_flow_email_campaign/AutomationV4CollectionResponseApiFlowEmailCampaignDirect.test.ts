...
describe('AutomationV4CollectionResponseApiFlowEmailCampaignDirect', async () => {
  ...
  test('direct-list-automation_v4_collection_response_api_flow_email_campaign', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-automation_v4_collection_response_api_flow_email_campaign', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_AUTOMATION_V4_COLLECTION_RESPONSE_API_FLOW_EMAIL_CAMPAIGN_ENTID']
