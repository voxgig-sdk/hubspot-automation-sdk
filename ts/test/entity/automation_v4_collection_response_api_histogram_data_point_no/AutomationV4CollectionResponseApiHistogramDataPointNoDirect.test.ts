...
describe('AutomationV4CollectionResponseApiHistogramDataPointNoDirect', async () => {
  ...
  test('direct-load-automation_v4_collection_response_api_histogram_data_point_no', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-automation_v4_collection_response_api_histogram_data_point_no', setup.live)) return
    if (skipIfMissingIds(t, setup, ["flow_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_AUTOMATION_V4_COLLECTION_RESPONSE_API_HISTOGRAM_DATA_POINT_NO_ENTID']
