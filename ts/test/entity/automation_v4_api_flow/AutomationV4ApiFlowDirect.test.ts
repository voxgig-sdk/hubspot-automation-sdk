...
describe('AutomationV4ApiFlowDirect', async () => {
  ...
  test('direct-load-automation_v4_api_flow', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-automation_v4_api_flow', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_AUTOMATION_V4_API_FLOW_ENTID']
