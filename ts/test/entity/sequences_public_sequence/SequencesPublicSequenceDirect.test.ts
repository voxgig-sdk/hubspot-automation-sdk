...
describe('SequencesPublicSequenceDirect', async () => {
  ...
  test('direct-load-sequences_public_sequence', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-sequences_public_sequence', setup.live)) return
    if (skipIfMissingIds(t, setup, ["sequence_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_SEQUENCES_PUBLIC_SEQUENCE_ENTID']
