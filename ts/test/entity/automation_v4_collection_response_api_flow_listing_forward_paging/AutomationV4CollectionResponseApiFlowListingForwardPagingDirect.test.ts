...
describe('AutomationV4CollectionResponseApiFlowListingForwardPagingDirect', async () => {
  ...
  test('direct-list-automation_v4_collection_response_api_flow_listing_forward_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-automation_v4_collection_response_api_flow_listing_forward_paging', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_AUTOMATION_TEST_AUTOMATION_V4_COLLECTION_RESPONSE_API_FLOW_LISTING_FORWARD_PAGING_ENTID']
