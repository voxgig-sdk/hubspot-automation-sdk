# HubspotAutomation JavaScript SDK



The JavaScript SDK for the HubspotAutomation API — an entity-oriented client with full async/await support.
...
## Install
```js
npm install hubspot-automation
```
...
### Load an ActionsV4CollectionResponsePublicActionDefinitionForward
...
console.log(actions_v4_collection_response_public_action_definition_forward)
```
...
| --- | --- | --- |
| `paging` | `Object` | Paging information for forward-only pagination. |
...
| `labels` | `Object` | An object containing labels for the action, with each property being a PublicActionLabels object. |
| `objectRequestOptions` | `Object` |  |
...
| `outputFields` | `Object` | Contains the output fields associated with the callback, with each field represented as a key-value pair. |
| `requestContext` | `*` | Specifies the context in which the request is made, which can be one of several predefined contexts. |
| `typedOutputs` | `Object` | Holds the typed outputs related to the callback, structured as an object. |
...
hubspot-automation/
├── src/
│   ├── HubspotAutomationSDK.js        # Main SDK class
